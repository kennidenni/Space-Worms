...
public class Graph {
    ...
    public void printGoalResult(int start, int goal) {
        String output = "Number of nodes = " + this.noOfNodes;
        output += "\nNumber of edges = " + this.noOfEdges;
        output += ("\nThe shortest distance from Square 1 to the goal on Square " + goal + " is " + nodes[goal - 1].getDistanceFromSource());

        System.out.println(output);

        System.out.print("The path in reversed order with start and goal + wormhole: " + goal + " ");
        printPath(goal, start);
    }

    private void printPath(int start, int goal) {
        if (start != goal) {
            System.out.print((start + 1) + " ");
            if (nodes[start].getParent() != -1) {
